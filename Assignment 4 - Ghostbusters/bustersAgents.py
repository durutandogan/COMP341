...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for dist in livingGhostPositionDistributions: 
            mostLikelyPos.append(dist.argMax())

        minDistance = float("inf") 
        minimizingAction = Directions.STOP 
       
        for action in legal:
            succ = Actions.getSuccessor(pacmanPosition, action)

            for ghost in mostLikelyPos:
                dist = self.distancer.getDistance(succ, ghost)

                if (dist < minDistance): #updating distance and aciton when min distance found
                    minDistance = dist
                    minimizingAction = action


        return minimizingAction
